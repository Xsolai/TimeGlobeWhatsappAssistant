...
// Utility function to format datetime (backend provides correct time)
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          if (dateRanges && dateRanges.available_dates) {
            setAvailableDates(dateRanges.available_dates);
          } else {
            setAvailableDates([]);
            console.warn('Keine gültigen dateRanges.available_dates empfangen.');
          }
        } catch (error) {
  ...
  const handleDateChange = async (newDate: Date | null) => {
    if (newDate) {
      try {
        ...
        // Überprüfe, ob das ausgewählte Datum in der Zukunft liegt (nur für Tages-Kalender)
        if (activeCalendarType !== 'month') {
          const today = new Date();
          ...
          <Box sx={{
            ...
            <Box sx={{
              ...
              {dashboardData?.recent_appointments && dashboardData.recent_appointments.length > 0 ? (() => {
                const activities = dashboardData.recent_appointments;
                console.log('[DEBUG] Rohdaten recent_appointments:', activities);
                
                // Schritt 1: Aktivitäten nach booking_id gruppieren
                const groupedByBookingId = activities.reduce((acc, activity) => {
                  const key = activity.booking_id; // Annahme: booking_id ist die Termin-ID
                  
                  // Service-Namen von führenden Nummern bereinigen (z.B. "01. Name" -> "Name")
                  const cleanedServiceName = activity.service_name.replace(/^\d{2}\.\s+/, '');

                  if (!acc[key]) {
                    acc[key] = {
                      ...activity, // Übernimmt Kundendaten, Datum, Zeit vom ersten Service
                      service_names: [cleanedServiceName], // Verwendet den bereinigten Service-Namen
                    };
                  } else {
                    // Füge Service hinzu, wenn er (bereinigt) nicht bereits in der Liste ist
                    if (!acc[key].service_names.includes(cleanedServiceName)) {
                        acc[key].service_names.push(cleanedServiceName);
                    }
                  }
                  return acc;
                }, {} as Record<string, typeof activities[0] & { service_names: string[] }>);

                // Schritt 2: In ein Array umwandeln, sortieren und die neuesten 10 nehmen
                let activitiesArray = Object.values(groupedByBookingId);
                console.log('[DEBUG] Länge activitiesArray (nach Gruppierung):', activitiesArray.length, activitiesArray);

                // Sortierung: Neueste zuerst (absteigend nach Datum und Zeit)
                ...
                // Stelle sicher, dass nur eine Deklaration von processedActivities vorhanden ist.
                const processedActivities = activitiesArray.slice(0, 10);
                console.log('[DEBUG] Länge processedActivities (nach Slice):', processedActivities.length, processedActivities);
                ...
                if (processedActivities.length === 0) {
                  return (
                    <Box sx={{ 
                      ...
                      <AccessTimeOutlined sx={{ fontSize: 48, color: 'rgba(0,0,0,0.2)', mb: 2 }} />
                      <Typography variant="body2" color="text.secondary">
                      ...
                      >
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                          <Typography variant="caption" sx={{
                            ...
                            {formatDateTime(activity.appointment_date, activity.appointment_time)} Uhr
                          </Typography>
                        </Box>
                      </Box>
                    ))}
